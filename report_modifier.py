...
def process_all_html_files(config: Dict) -> None:
    global ORIGINAL_ROOT, NEW_ROOT, SWE4_JSON_ROOT, OVERWRITE_NEW_ROOT, RECURSIVE_SEARCH
    global TEST_START_DATE, EXECUTION_START_DATE, DATE_FORMAT, RANDOM_SEED
    global CREATION_MIN_STEP_MINUTES, CREATION_MAX_STEP_MINUTES, EXECUTION_STEP_PER_TC_COUNT, EXECUTION_STEP_MINUTES
    global JSON_FILE_MAP, ENABLE_TIME_LOG
    ORIGINAL_ROOT = config["ORIGINAL_ROOT"]
    NEW_ROOT = config["NEW_ROOT"]
    SWE4_JSON_ROOT = config["SWE4_JSON_ROOT"]
    OVERWRITE_NEW_ROOT = config["OVERWRITE_NEW_ROOT"]
    RECURSIVE_SEARCH = config["RECURSIVE_SEARCH"]
    TEST_START_DATE = config["TEST_START_DATE"]
    EXECUTION_START_DATE = config["EXECUTION_START_DATE"]
    DATE_FORMAT = config["DATE_FORMAT"]
    RANDOM_SEED = config["RANDOM_SEED"]
    CREATION_MIN_STEP_MINUTES = config["CREATION_MIN_STEP_MINUTES"]
    CREATION_MAX_STEP_MINUTES = config["CREATION_MAX_STEP_MINUTES"]
    EXECUTION_STEP_PER_TC_COUNT = config["EXECUTION_STEP_PER_TC_COUNT"]
    EXECUTION_STEP_MINUTES = config["EXECUTION_STEP_MINUTES"]
    JSON_FILE_MAP = config["JSON_FILE_MAP"]
